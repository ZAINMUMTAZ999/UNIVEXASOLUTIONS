...
import { User, UserIcon, X, Plus, Star } from "lucide-react";
...
export const GetallReviews = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<addReviewTypes[], Error>("reviews", GetAllReviewsApi);
...
<div className="w-full flex justify-end">
  ...
  >
    ...
    <span>Add Review</span>
      ...
      <button
        onClick={() => scroll("left")}
        className="hidden sm:flex absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-gray-50 shadow-lg border border-gray-200 w-10 h-10 rounded-full items-center justify-center transition-all duration-200"
      >
        <span className="text-gray-600 text-lg">←</span>
      </button>
      <button
        onClick={() => scroll("right")}
        className="hidden sm:flex absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-white hover:bg-gray-50 shadow-lg border border-gray-200 w-10 h-10 rounded-full items-center justify-center transition-all duration-200"
      >
        <span className="text-gray-600 text-lg">→</span>
      </button>
